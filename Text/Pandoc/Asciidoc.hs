...
data QuoteType = Strong | Emphasis | Unquoted | Strikeout | Mono
    deriving (Show)
...
pandoc (h, preamble,lst) = P.Pandoc meta $ concatMap toPandocA lst
  where
...
pandocListItem ::  ListNode -> [P.Block]
pandocListItem (ListNode item []) = [P.Plain $ pandocInlineMap item]
pandocListItem (ListNode item children) = [P.Plain $ pandocInlineMap item, P.BulletList $ map pandocListItem children]

toPandocA ::  Attributed Block -> [P.Block]
toPandocA blk =
  case title blk of
    Just tt -> [P.Para [P.Strong [P.Str tt]], toPandoc (content blk)]
    Nothing -> [toPandoc (content blk)]

toPandoc ::  Block -> P.Block
toPandoc (Header n str) = P.Header n [P.Str str]
toPandoc (Para lst) = P.Para $ pandocInlineMap lst
toPandoc (AdmPara t lst) = P.BlockQuote [P.Para $ [P.Strong [P.Str $ show t], P.Str ":", P.Space] ++ pandocInlineMap lst]
toPandoc (Code lst) = P.CodeBlock nullAttr $ intercalate "\n" lst
toPandoc (BulletedList lst) = P.BulletList $ map pandocListItem lst
toPandoc (NumberedList lst) = P.OrderedList defListAttr [[P.Plain $ pandocInlineMap item] | item <- lst]
toPandoc (DefList lst) = P.DefinitionList [(pandocInlineMap t, [[P.Plain $ pandocInlineMap d]]) | (t,d) <- lst]
toPandoc (Delimited c lst) = P.BlockQuote [P.Plain $ pandocInlineMap line | line <- lst]
toPandoc (Table lst) = P.Table [] (map alignment $ head lst) [] [] [[pandocCell cell | cell <- row] | row <- lst]
  where
    ...
    cell' lst = [P.Plain $ pandocInlineMap lst]

pandocInlineMap ::  [Inline] -> [P.Inline]
pandocInlineMap = concatMap pandocInline

pandocInline :: Inline -> [P.Inline]
pandocInline (Text s) = [P.Space, P.Str s]
pandocInline (Macro name arg args) = 
    case name of
      "link" -> [P.Space, P.Link (map P.Str args) (arg,"")]
      "image" -> [P.Image (map P.Str args) (arg,"")]
      "include" -> [P.Space, P.Link [P.Str $ tail arg] (dropExtension $ tail arg,"")]
      _ -> [P.Str "<unknown macro>"]
pandocInline (Quoted qt s) = 
    case qt of
...
pandocInline (InternalLink h t) = [P.Space, P.Link [P.Str t] ("#"++h,"")]

readAsciidoc :: String -> Pandoc.Pandoc
readAsciidoc text = case parse asciidoc "<input>" text' of
    Left e -> error $ show e
    Right x -> pandoc x
  where
    text' | last text == '\n' = text
